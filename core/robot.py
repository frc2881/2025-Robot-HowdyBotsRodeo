...
class RobotCore:
  ...
  def _setupOperator(self) -> None:
    ...
    self.operator.leftTrigger().whileTrue(self.intake.intake())
    self.operator.rightTrigger().whileTrue(self.intake.eject())
    # self.operator.rightBumper().whileTrue(cmd.none())
    # self.operator.leftBumper().whileTrue(cmd.none())
    self.operator.povUp().whileTrue(self.arm.setPosition(constants.Subsystems.Arm.kArmPositionEjectScrap))
    self.operator.povDown().whileTrue(self.arm.setPosition(constants.Subsystems.Arm.kArmPositionIntake))
    # self.operator.povLeft().whileTrue(cmd.none())
    self.operator.povRight().whileTrue(self.arm.setPosition(constants.Subsystems.Arm.kArmPositionScoreSalvage))
