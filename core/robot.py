...
class RobotCore:
  ...
  def _setupOperator(self) -> None:
    ...
    self.operator.povUp().whileTrue(self.arm.setPosition(0.22))
    self.operator.povDown().whileTrue(self.arm.setPosition(0.04))
    # self.operator.povLeft().whileTrue(cmd.none())
    self.operator.povRight().whileTrue(self.arm.setPosition(0.09))
